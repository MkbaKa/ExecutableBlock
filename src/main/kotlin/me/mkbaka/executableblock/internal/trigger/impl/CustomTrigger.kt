...
class CustomTrigger(val config: Configuration, val section: String) : BukkitTrigger() {
    ...
    override fun condition(event: EventAdapter): Boolean {
        ...
        val script = config.getString("$section.override.condition")!!
        if (script.contains("super.condition()")) script.replace("super.condition()", super.condition(event).toString())
